...
# Replace with less safe, looks for digital signature of Microsoft instead. 
<#
...
function verifyPackage ($file, $certificate, $thumbprint, $name, $url) {
    ...
    if (-not $signature.SignerCertificate) {
        Write-Host "! ERROR: $name installer has no digital signature."
        exit 1
    }

    if ($signature.Status -ne "Valid") {
        Write-Host "! ERROR: $name installer signature check failed."
        Write-Host "  Status: $($signature.Status)"
        exit 1
    }

    if ($signature.SignerCertificate.Subject -notmatch "Microsoft Corporation") {
        Write-Host "! ERROR: $name installer is not signed by Microsoft."
        Write-Host "  Subject: $($signature.SignerCertificate.Subject)"
        exit 1
    }

    Write-Host "- Digital Signature verification passed (signed by Microsoft)."
}
